#!/usr/bin/env python3
"""Bounded clean-room analysis of the final /load/index user_chara_list block.

The exact user_chara_list literal xref lands at 0x485D17C. This helper inspects
only the bounded region ending immediately before the independently known second
user_unit_list entry at 0x485DE38. It emits named direct calls plus tiny contexts
around exact string-literal accesses, not the complete LoadTask.Parse body.
"""
...
def literal_hits(view: BinaryView, instructions: list[Any], literals: dict[int, str]):
    ...
    for start_index, first in enumerate(instructions):
        if first.id not in {ARM64_INS_ADR, ARM64_INS_ADRP}:
            continue
        state: dict[int, int] = {}
        for ins in instructions[start_index : min(len(instructions), start_index + WINDOW_INSTRUCTIONS)]:
            ops = ins.operands
            ...
            if ins.id == ARM64_INS_LDR and len(ops) >= 2:
                if ops[0].type == ARM64_OP_REG and ops[1].type == ARM64_OP_MEM:
                    ...
                    if base is not None and int(mem.index) == 0:
                        ...
                        if loaded in literal_addresses:
                            raw.setdefault(
                                (ins.address, loaded),
                                {
                                    "site": ins.address,
                                    "literal_address": loaded,
                                    "value": literals[loaded],
                                },
                            )
    ...
    if len(raw) > MAX_LITERAL_HITS:
        raise RuntimeError(f"bounded region exposes too many literal hits: {len(raw)}")
    return list(raw.values())


def context_for(instructions: list[Any], site: int) -> list[str]:
    ...
    return [fmt(ins) for ins in instructions[lo:hi]]
...
def main() -> int:
    ...
    for hit in hits:
        hit["context"] = context_for(instructions, hit["site"])
    ...
    for ins in instructions:
        ...
        if name is not None:
            calls.append({"site": ins.address, "target": target, "name": name})
    if len(calls) > MAX_NAMED_CALLS:
        raise RuntimeError(f"too many named calls in bounded block: {len(calls)}")

    report = {
        "schema": 1,
        "region": {"start": START_RVA, "end": END_RVA, "size": END_RVA - START_RVA},
        "literal_hit_count": len(hits),
        "literal_hits": sorted(hits, key=lambda item: item["site"]),
        "named_call_count": len(calls),
        "named_calls": calls,
    }
    if not any(hit["value"] == "user_chara_list" for hit in hits):
        raise RuntimeError("bounded region failed to resolve user_chara_list")
    ...
    print(
        json.dumps(
            {
                "literal_values": [hit["value"] for hit in sorted(hits, key=lambda item: item["site"])],
                "named_calls": [call["name"] for call in calls],
            },
            ensure_ascii=False,
        )
    )
