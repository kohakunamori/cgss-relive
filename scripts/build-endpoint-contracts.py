...
def direct_bindings(field_report: dict[str, Any]) -> tuple[dict[int, list[dict[str, Any]]], set[str]]:
    ...
    for task in field_report.get("tasks", []):
        ...
        for key in sorted(set(values)):
            if 0 <= key <= 515:
                bindings[key].append({"task":task_name,"evidence":"direct-networktask-type-write"})
...
def add_arcade_bindings(bindings: dict[int, list[dict[str, Any]]], arcade_report: dict[str, Any]) -> None:
    for row in arcade_report.get("constructors", []):
        key = int(row["convert_input_key"]); expected = int(row["expected_api_key"])
        if key != expected:
    ...
    for group in ("A", "B"):
        for endpoint in api_map[group]:
            ...
            for binding in raw_bindings:
                ...
                if marker in seen:
                    continue
                seen.add(marker); unique.append(binding)
            candidates = []
            if not unique:
            ...
            row = dict(endpoint); row.update({"status":status,"task_bindings":unique,"name_candidates":candidates})
